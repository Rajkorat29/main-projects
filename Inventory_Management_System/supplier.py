...
def supplier_page(root, mycursor, conn):
    ...
    # GUI
    supplier_window = Frame(root, width=1325, height=755, bg='#E4E0E1')
    supplier_window.place(x=200, y=100)


    titleLabel = Label(supplier_window, text='Manage Supplier Details', font=('times new roman', 16, 'bold'), bg='#493628',fg='#E4E0E1')
    titleLabel.place(x=0, y=0, relwidth=1)

    backImage = PhotoImage(file='back.png')
    backButton = Button(supplier_window, image=backImage, bd=0, bg='#E4E0E1', cursor='hand2', command=back_func)
    backButton.image = backImage
    backButton.place(x=10, y=35)


    leftFrame = Frame(supplier_window, bg='#E4E0E1')
    leftFrame.place(x=10, y=100)

    invoiceLabel = Label(leftFrame, text='Invoice No.', font=('times new roman', 14,'bold'), bg='#E4E0E1')
    invoiceLabel.grid(row=0, column=0, sticky='w', padx=(20,40))
    invoiceEntry = Entry(leftFrame, font=('times new roman', 13), bg='white')
    invoiceEntry.grid(row=0, column=1, pady=20, sticky='w')

    suppliernameLabel = Label(leftFrame, text='Supplier Name', font=('times new roman', 14,'bold'), bg='#E4E0E1')
    suppliernameLabel.grid(row=1, column=0, sticky='w', padx=(20,40))
    suppliernameEntry = Entry(leftFrame, font=('times new roman', 13), bg='white')
    suppliernameEntry.grid(row=1, column=1, sticky='w')

    contactLabel = Label(leftFrame, text='Contact', font=('times new roman', 14,'bold'), bg='#E4E0E1')
    contactLabel.grid(row=2, column=0, sticky='w', padx=(20,40))
    contactEntry = Entry(leftFrame, font=('times new roman', 13), bg='white')
    contactEntry.grid(row=2, column=1, pady=20, sticky='w')

    descriptionLabel = Label(leftFrame, text='Description', font=('times new roman', 14,'bold'), bg='#E4E0E1')
    descriptionLabel.grid(row=3, column=0, sticky='nw', padx=(20,40))
    descriptionText = Text(leftFrame, font=('times new roman', 14), bg='white', width=35, height=12)
    descriptionText.grid(row=3, column=1)

    # buttons
    buttonFrame = Frame(supplier_window, bg='#E4E0E1')
    buttonFrame.place(x=140, y=530)

    saveButton = Button(buttonFrame, text='Save', font=('times new roman', 12, 'bold'), width=8, fg='white', bg='#493628', cursor='hand2', command=save_data)
    ...
    searchFrame = Frame(supplier_window, bg='#E4E0E1')
    searchFrame.place(x=700,y=95,width=500,height=340)

    invoiceLabel = Label(searchFrame, text='Invoice No.', font=('times new roman', 14,'bold'), bg='#E4E0E1')
    invoiceLabel.grid(row=0, column=0, padx=20)

    searchEntry = Entry(searchFrame, font=('times new roman', 13), bg='white', width=14)
    searchEntry.grid(row=0, column=1)

    searchButton = Button(searchFrame, text='Search', font=('times new roman', 12, 'bold'), bg='#493628', fg='white',cursor='hand2', width=8, command=search)
    searchButton.grid(row=0, column=2, padx=20)

    showButton = Button(searchFrame, text='Show All', font=('times new roman', 12, 'bold'), width=8, fg='white',bg='#493628', cursor='hand2', command=show_data)
    showButton.grid(row=0, column=3)

    treeviewFrame = Frame(supplier_window, bg='white')
    treeviewFrame.place(x=700, y=165, height=400,width=510)

    scrolly = Scrollbar(treeviewFrame, orient=VERTICAL)
    scrollx = Scrollbar(treeviewFrame, orient=HORIZONTAL)
    treeview = ttk.Treeview(treeviewFrame, columns=('invoice', 'name', 'contact', 'description'),yscrollcommand=scrolly.set, xscrollcommand=scrollx.set)
