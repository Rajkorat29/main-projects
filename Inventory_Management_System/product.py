...
def product_page(root, mycursor, conn):
    ...
    product_window = Frame(root, width=1325, height=755, bg='#E4E0E1')
    product_window.place(x=200, y=100)

    titleLabel = Label( product_window, text='Manage Products', font=('times new roman', 16, 'bold'), bg='#493628',fg='#E4E0E1')
    titleLabel.place(x=0, y=0, relwidth=1)

    backImage = PhotoImage(file='back.png')
    backButton = Button(product_window, image=backImage, bd=0,  bg='#E4E0E1', cursor='hand2',
                        command=lambda: product_window.destroy())
    backButton.image = backImage
    backButton.place(x=10, y=35)

    leftFrame = Frame(product_window, bd=2, relief=RIDGE, bg='#E4E0E1')
    leftFrame.place(x=60, y=60)

    titleLabel = Label(leftFrame, text='Manage Product Details', font=('times new roman', 15),bg='#493628',fg='#E4E0E1')
    titleLabel.grid(row=0, column=0, columnspan=2, sticky='news')

    categoryLabel = Label(leftFrame, text='Category', font=('times new roman', 15,), bg='#E4E0E1')
    categoryLabel.grid(row=1, column=0, sticky='w', padx=20)
    categoryCombobox = ttk.Combobox(leftFrame,font=('times new roman', 15),state='readonly', width=18)
    categoryCombobox.grid(row=1, column=1, pady=15, sticky='w')
    categoryCombobox.set('Empty')

    supplierLabel = Label(leftFrame, text='Supplier', font=('times new roman', 15),  bg='#E4E0E1')
    supplierLabel.grid(row=2, column=0, sticky='w', padx=20)
    supplierCombobox = ttk.Combobox(leftFrame,font=('times new roman', 15),state='readonly', width=18)
    supplierCombobox.grid(row=2, column=1, pady=15, sticky='w')
    supplierCombobox.set('Empty')

    nameLabel = Label(leftFrame, text='Name', font=('times new roman', 15),  bg='#E4E0E1')
    ...
    priceLabel = Label(leftFrame, text='Price', font=('times new roman', 15),  bg='#E4E0E1')
    ...
    discountLabel = Label(leftFrame, text='Discount (%)', font=('times new roman', 15),  bg='#E4E0E1')
    ...
    quantityLabel = Label(leftFrame, text='Quantity ', font=('times new roman', 15),  bg='#E4E0E1')
    ...
    statusLabel = Label(leftFrame, text='Status', font=('times new roman', 15),  bg='#E4E0E1')
    ...
    # buttons
    buttonFrame = Frame(leftFrame,  bg='#E4E0E1')
    ...
    saveButton = Button(buttonFrame, text='Save', font=('times new roman', 12, 'bold'), width=8, fg='white',
                        bg='#493628', cursor='hand2', command=save_data)
    saveButton.grid(row=0, column=0, padx=8)
    updateButton = Button(buttonFrame, text='Update', font=('times new roman', 12, 'bold'), width=8, fg='white',bg='#493628', cursor='hand2', command=update_data)
    updateButton.grid(row=0, column=1, padx=8)

    deleteButton = Button(buttonFrame, text='Delete', font=('times new roman', 12, 'bold'), width=8, fg='white',bg='#493628', cursor='hand2', command=delete_data)
    deleteButton.grid(row=0, column=2, padx=8)

    clearButton = Button(buttonFrame, text='Clear', font=('times new roman', 12, 'bold'), width=8, fg='white',bg='#493628', cursor='hand2', command=lambda: clear(True))
    clearButton.grid(row=0, column=3, padx=8)

    searchFrame = LabelFrame(product_window,  bg='#E4E0E1', text='Search Products', font=('times new roman', 13, 'bold'))
    searchFrame.place(x=655, y=50)

    searchCombobox = ttk.Combobox(searchFrame, values=('Category', 'Supplier', 'Name'),font=('times new roman', 12),state='readonly')
    searchCombobox.grid(row=0, column=0, padx=10)
    searchCombobox.set('Search By')

    searchEntry = Entry(searchFrame, font=('times new roman', 12), bg='white')
    searchEntry.grid(row=0, column=1, padx=(0, 10))

    searchButton = Button(searchFrame, text='Search', font=('times new roman', 12, 'bold'), bg='#493628', fg='white',cursor='hand2', width=10, command=search)
    searchButton.grid(row=0, column=2, pady=5)

    showButton = Button(searchFrame, text='Show All', font=('times new roman', 12, 'bold'), width=10, fg='white',bg='#493628', cursor='hand2', command=show_data)
    showButton.grid(row=0, column=3, pady=5, padx=10)

    treeviewFrame = Frame(product_window,  bg='#E4E0E1')
    treeviewFrame.place(x=655, y=130, height=455, width=600)

    scrolly = Scrollbar(treeviewFrame, orient=VERTICAL)
    scrollx = Scrollbar(treeviewFrame, orient=HORIZONTAL)
    treeview = ttk.Treeview(treeviewFrame,columns=('id', 'category', 'supplier', 'name', 'price','discount', 'price_after_discount', 'quantity', 'status'),yscrollcommand=scrolly.set, xscrollcommand=scrollx.set)
