...
def connection():
    ...
    try:
        conn = pymysql.connect(host='localhost', user='root', password='1247')
    ...
    mycursor.execute('CREATE TABLE IF NOT EXISTS sup_data (invoice INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(50), contact VARCHAR(15), description VARCHAR(150))')
    mycursor.execute('CREATE TABLE IF NOT EXISTS emp_data (empid INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(50), email VARCHAR('
'50), gender VARCHAR(20), dob VARCHAR(30), contact VARCHAR(30), employment_type VARCHAR(50), education VARCHAR(50), '
    ...
    mycursor.execute("""CREATE TABLE IF NOT EXISTS settings (  id INT AUTO_INCREMENT PRIMARY KEY, tax_percentage FLOAT)""")
...
def login(event=None):
    if empIdEntry.get() == '' or passwordEntry.get() == '':
        ...
        if check_empty_emp_data():
            ...
            if empIdEntry.get() == '1' and passwordEntry.get() == '1247':
...
title = Label(root, text='Inventory Management System', font=('times new roman', 40,'bold'),bg='#493628',fg='#E4E0E1')
title.place(x=0, y=0, relwidth=1)
logoLabel = Label(root, bg='#E4E0E1')
logoLabel.place(x=200, y=170)
rightFrame = Frame(root, bg='white', height=500, width=400)
rightFrame.place(x=900, y=180)
logo = PhotoImage(file='login_man.png')
employeelogoLabel = Label(rightFrame, image=logo, bg='white', bd=0)
employeelogoLabel.grid(row=0, column=0, pady=20)

empIdLabel = Label(rightFrame, text='Employee Id', font=('times new roman', 15), bg='white')
empIdLabel.grid(row=1, column=0, sticky='w', padx=50)
empIdEntry = Entry(rightFrame, font=('times new roman', 15),bg='#E4E0E1')
empIdEntry.grid(row=2, column=0, sticky='w', padx=50)

passwordLabel = Label(rightFrame, text='Password', font=('times new roman', 15), bg='white')
passwordLabel.grid(row=3, column=0, sticky='w', padx=50, pady=(10, 0))

passwordEntry = Entry(rightFrame, font=('times new roman', 15),bg='#E4E0E1', show='*')
...
toggle_button = Button(rightFrame, image=close_eye_img, command=toggle_password, bd=0, bg='white',
                       activebackground='white')
toggle_button.place(x=260, y=280)

forgetpassButton = Button(rightFrame, text='Forgot Password?', font=('times new roman', 12), bd=0, fg='#AB886B',
                          cursor='hand2', bg='white', activebackground='white',
                          command=forget_password)
forgetpassButton.grid(row=5, column=0, sticky='w', padx=45)

loginButton = Button(rightFrame, text='Login', font=('times new roman', 14), width=20, bg='#493628', cursor='hand2',
                     fg='white',
                     command=login)
