...
def employee_page(root, mycursor, conn):
    ...
    def search():
        if searchEntry.get() == '':
            ...
            mycursor.execute(f'SELECT * FROM emp_data WHERE {search_combobox.get()} LIKE %s','%' + searchEntry.get() + '%')
    ...
    #  gui
    employee_window = Frame(root, width=1325, height=755, bg='#E4E0E1')
    employee_window.place(x=200, y=100)
    titleLabel = Label(employee_window, text='Manage Employee Details', font=('times new roman', 16, 'bold'),bg='#493628',fg='#E4E0E1')
    titleLabel.place(x=0, y=0, relwidth=1)
    backImage = PhotoImage(file='back.png')
    backButton = Button(employee_window, image=backImage, bd=0, cursor='hand2', bg='#E4E0E1', command=back_func)
    backButton.image = backImage
    backButton.place(x=10, y=35)



    treeviewFrame = Frame(employee_window, bg='#E4E0E1')
    treeviewFrame.place(x=0, y=70, relwidth=1, height=230,anchor='nw')

    searchFrame = Frame(treeviewFrame, bg='#E4E0E1')
    searchFrame.pack(pady=(10))
    search_combobox = ttk.Combobox(searchFrame, values=('Empid', 'Name', 'Email', 'employment_type', 'work_shift', 'Education', 'Salary'), state='readonly',justify=CENTER, font=('goudy old style', 12))
    search_combobox.grid(row=0, column=0, padx=20)
    search_combobox.set('Search By')
    searchEntry = Entry(searchFrame, font=('goudy old style', 12), bg='white')
    searchEntry.grid(row=0, column=1)
    searchButton = Button(searchFrame, text='Search', font=('times new roman', 12,),bg='#493628',fg='#E4E0E1',cursor='hand2', width=10, command=search)
    searchButton.grid(row=0, column=2, padx=20, pady=10)
    showButton = Button(searchFrame, text='Show All', font=('times new roman', 12), width=10,bg='#493628',fg='#E4E0E1', cursor='hand2', command=show_data)
    ...
    treeview = ttk.Treeview(treeviewFrame, columns=('empid', 'name', 'email', 'gender', 'dob', 'contact', 'employment_type', 'education', 'work_shift', 'address', 'doj','salary', 'usertype',),yscrollcommand=scrolly.set, xscrollcommand=scrollx.set)
    ...
    # detailsframe
    detailsFrame = Frame(employee_window, bg='#E4E0E1')
    detailsFrame.place(x=150, y=330)

    empIdLabel = Label(detailsFrame, text='EmpId', font=('times new roman', 12), bg='#E4E0E1')
    empIdLabel.grid(row=0, column=0, padx="20",pady='10', sticky='w')
    empIdEntry = Entry(detailsFrame, font=('times new roman', 12), bg='white')
    empIdEntry.grid(row=0, column=1, padx="20",pady='10')
    
    nameLabel = Label(detailsFrame, text='Name', font=('times new roman', 12), bg='#E4E0E1')
    nameLabel.grid(row=0, column=2, padx="20",pady='10', sticky='w')
    nameEntry = Entry(detailsFrame, font=('times new roman', 12), bg='white')
    nameEntry.grid(row=0, column=3,padx="20",pady='10')
    
    emailLabel = Label(detailsFrame, text='Email', font=('times new roman', 12), bg='#E4E0E1')
    emailLabel.grid(row=0, column=4, padx="20",pady='10', sticky='w')
    emailEntry = Entry(detailsFrame, font=('times new roman', 12), bg='white')
    emailEntry.grid(row=0, column=5,padx="20",pady='10')
    
    genderLabel = Label(detailsFrame, text='Gender', font=('times new roman', 12), bg='#E4E0E1')
    genderLabel.grid(row=1, column=0, padx="20",pady='10', sticky='w')
    genderCombobox = ttk.Combobox(detailsFrame, values=('Male', 'Female'), font=('times new roman', 12), state='readonly', width=18)
    genderCombobox.grid(row=1, column=1,padx="20",pady='10')
    genderCombobox.current(0)

    dobLabel = Label(detailsFrame, text='Date of Birth', font=('times new roman', 12), bg='#E4E0E1')
    dobLabel.grid(row=1, column=2, padx="20",pady='10', sticky='w')
    dobDateEntry = DateEntry(detailsFrame, font=('times new roman', 12), bg='white', width=18, date_pattern='dd/MM/yyyy', state='readonly')
    dobDateEntry.grid(row=1, column=3,padx="20",pady='10')
    
    contactLabel = Label(detailsFrame, text='Contact', font=('times new roman', 12), bg='#E4E0E1')
    contactLabel.grid(row=1, column=4, padx="20",pady='10', sticky='w')
    contactEntry = Entry(detailsFrame, font=('times new roman', 12), bg='white')
    contactEntry.grid(row=1, column=5,padx="20",pady='10')
    
    employment_typeLabel = Label(detailsFrame, text='Employment Type', font=('times new roman', 12), bg='#E4E0E1')
    employment_typeLabel.grid(row=2, column=0, padx="20",pady='10', sticky='w')
    employment_typeCombobox = ttk.Combobox(detailsFrame, values=('Full Time', 'Part Time', 'Casual','Contract','Intern'),font=('times new roman', 12),state='readonly', width=18)
    employment_typeCombobox.grid(row=2, column=1, padx="20",pady='10')
    employment_typeCombobox.current(0)

    educationLabel = Label(detailsFrame, text='Education', font=('times new roman', 12), bg='#E4E0E1')
    educationLabel.grid(row=2, column=2, padx="20",pady='10', sticky='w')
    education_options = ["B.Tech", "B.Com", "M.Tech", "M.Com", "B.Sc", "M.Sc", "BBA", "MBA", "LLB", "LLM", "B.Arch", "M.Arch"]
    educationBox = ttk.Combobox(detailsFrame, values=education_options, width=18, font=('times new roman', 12),state='readonly')
    educationBox.grid(row=2, column=3,padx="20",pady='10')
    educationBox.set(education_options[0])
    
    work_shiftLabel = Label(detailsFrame, text='Work Shift', font=('times new roman', 12), bg='#E4E0E1')
    work_shiftLabel.grid(row=2, column=4, padx="20",pady='10', sticky='w')
    ...
    work_shiftBox.grid(row=2, column=5,padx="20",pady='10')
    work_shiftBox.set(work_shift_options[0])

    addressLabel = Label(detailsFrame, text='Address', font=('times new roman', 12), bg='#E4E0E1')
    addressLabel.grid(row=3, column=0, padx="20",pady='10', sticky='w')
    addressText = Text(detailsFrame, width=20, height=3, font=('times new roman', 12), bd=2,bg='white')
    addressText.grid(row=3, column=1, rowspan=2,padx="20",pady='10')
    
    
    dojLabel = Label(detailsFrame, text='Date of Joining', font=('times new roman', 12), bg='#E4E0E1')
    dojLabel.grid(row=3, column=2, padx="20",pady='10', sticky='w')
    dojDateEntry = DateEntry(detailsFrame, font=('times new roman', 12), bg='white', width=18, date_pattern='dd/MM/yyyy', state='readonly')
    dojDateEntry.grid(row=3, column=3,padx="20",pady='10')
   
    userTypeLabel = Label(detailsFrame, text='User Type', font=('times new roman', 12), bg='#E4E0E1')
    userTypeLabel.grid(row=4, column=2, padx="20",pady='10', sticky='w')
    userTypecombobox = ttk.Combobox(detailsFrame, font=('times new roman', 12), width=18, values=('Employee', 'Admin'), state='readonly')
    userTypecombobox.set('Select User Type')
    userTypecombobox.grid(row=4, column=3, padx="20",pady='10')
    userTypecombobox.current(0)
    
    salaryLabel = Label(detailsFrame, text='Salary', font=('times new roman', 12), bg='#E4E0E1')
    salaryLabel.grid(row=3, column=4, padx="20",pady='10', sticky='w')
    salaryEntry = Entry(detailsFrame, font=('times new roman', 12), bg='white')
    salaryEntry.grid(row=3, column=5,padx="20",pady='10')

    passwordLabel = Label(detailsFrame, text='Password', font=('times new roman', 12), bg='#E4E0E1')
    passwordLabel.grid(row=4, column=4, padx="20",pady='10', sticky='w')
    passwordEntry = Entry(detailsFrame, font=('times new roman', 12), bg='white')
    passwordEntry.grid(row=4, column=5, padx="20",pady='10')

    # buttons
    buttonFrame = Frame(employee_window, bg='#E4E0E1')
    buttonFrame.place(x=380, y=590)

    saveButton = Button(buttonFrame, text='Save', font=('times new roman', 12, 'bold'), width=10,bg='#493628',fg='#E4E0E1', cursor='hand2', command=save_data)
    saveButton.grid(row=0, column=0, padx=20)

    updateButton = Button(buttonFrame, text='Update', font=('times new roman', 12, 'bold'), width=10, bg='#493628',fg='#E4E0E1', cursor='hand2', command=update_data)
    updateButton.grid(row=0, column=1, padx=20)

    deleteButton = Button(buttonFrame, text='Delete', font=('times new roman', 12, 'bold'), width=10, bg='#493628',fg='#E4E0E1', cursor='hand2', command=delete_data)
    deleteButton.grid(row=0, column=2, padx=20)

    clearButton = Button(buttonFrame, text='Clear', font=('times new roman', 12, 'bold'), width=10,bg='#493628',fg='#E4E0E1', cursor='hand2', command=lambda: clear(True))
